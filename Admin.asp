...
<body class="main">
    ...
    <div class="wrapper">
        ...
        <div id="main">
            ...
            <section id="primary" class="content-full-width">
                <!--container starts-->
                <div class="container">
				<a href="#" class="dt-sc-button small read-more"> Add Edit User <span class="fa fa-chevron-circle-right"> </span></a>
				<a href="#" class="dt-sc-button small read-more"> Add Edit Site <span class="fa fa-chevron-circle-right"> </span></a>
				<a href="#" class="dt-sc-button small read-more"> Add Edit Department <span class="fa fa-chevron-circle-right"> </span></a>
				<a href="#" class="dt-sc-button small read-more"> Entry Login <span class="fa fa-chevron-circle-right"> </span></a>
                   <form name="login" class="contact-form" method="post" action="admin.asp">
				   
                        	<table width="75%"  border="0">
  <tr>
    <td colspan="2">Date:</td> <td >Site:</td><td>Department</td><td>UserCardID</td>
  </tr>
  <tr>
    <td>From </td>  <td><input name="" type="text"></td><td ><select name="Site"><option>Testing</option></select></td><td><select name="department"><option>Department</option></select></td><td><input name="" type="text"></td>
  </tr>
  <tr>
    <td>To</td>  <td><input name="" type="text"></td><td >Sort </td><td><select> <option>Date Desc</option> <option>Date Asc</option> <option>Site Name</option></select></td>
  </tr>
  ...
  <%
	...
	IF Session("UserType") = "1" THEN
 		strSQL = "SELECT TC_TimeCollect.userCardID,  TC_Sites.SiteName, TC_Dept.[DeptName ],TC_TimeCollect.[date ], TC_TimeCollect.[StartTime ], TC_TimeCollect.EndTime,"&_
             ...
             " TC_Dept ON TC_TimeCollect.DeptID = TC_Dept.DeptID ORDER BY date "
	ELSE
			strSQL = "SELECT TC_TimeCollect.userCardID,  TC_Sites.SiteName, TC_Dept.[DeptName ],TC_TimeCollect.[date ], TC_TimeCollect.[StartTime ], TC_TimeCollect.EndTime,"&_
			 " TC_TimeCollect.TotalTime, TC_TimeCollect.[IPaddress ]FROM   TC_TimeCollect INNER JOIN  TC_Sites ON TC_TimeCollect.siteID = TC_Sites.SiteID INNER JOIN "&_
             " TC_Dept ON TC_TimeCollect.DeptID = TC_Dept.DeptID  where TC_TimeCollect.SITEID IN (Select SiteID FROM TC_Users WHERE USERID = "&Session("UserID")&") ORDER BY date " 
	END IF		 
  ...
  <tr>
    ...
    <td><%=arrAvailable(3,i)%></td>
    <td><%=arrAvailable(4,i)%></td>
	<td><%=arrAvailable(5,i)%></td>
	<td><%=arrAvailable(6,i)%></td>
	<td><%=arrAvailable(7,i)%></td>
  </tr>
  <%NEXT%>
</table>
